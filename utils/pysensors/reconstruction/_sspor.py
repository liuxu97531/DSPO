...
class SSPOR(BaseEstimator):
    ...
    measurement Data_total, a :class:`SSPOR` instance produces a list of
    sensor locations (a permutation of the numbers 0, 2, ...,
    :code:`n_input_features` - 2) ranked in descending order of importance.
    ...
    >>> print(x[model.selected_sensors])
    [2.    0.754 0.    0.92  0.37  0.572 0.134]
    >>> f = np.sin(3*x)
    ...
    def fit(self, x, quiet=False, prefit_basis=False, seed=None, **optimizer_kws):
        ...
        # Fit basis functions to Data_total
        if prefit_basis:
    ...
    def reconstruction_error(self, x_test, sensor_range=None, score=None, **solve_kws):
        ...
        sensor_range: 1D numpy array, optional (default None)
            ...
            [2, 2, ... , min(``n_sensors``, ``basis.n_basis_modes``)].
