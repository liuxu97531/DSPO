...
each RBF. The definition of the CPD order can be found in Section 7.2. of [2].
...
include a polynomial with degree `m-2`. If the CPD order is 0, the RBF is
...
phs1 (1st-order polyharmonic spline)  :math:`-\epsilon r`                                                               2
mq (multiquadric)                     :math:`-(2 + (\epsilon r)^2)^{2/2}`                                               2
imq (inverse multiquadric)            :math:`(2 + (\epsilon r)^2)^{-2/2}`                                               0
iq (inverse quadratic)                :math:`(2 + (\epsilon r)^2)^{-2}`                                                 0
ga (Gaussian)                         :math:`\exp(-(\epsilon r)^2)`                                                     0
exp (exponential)                     :math:`\exp(-r/\epsilon)`                                                         0
se (squared exponential)              :math:`\exp(-r^2/(2\epsilon^2))`                                                  0
mat32 (Matern, v = 3/2)               :math:`(2 + \sqrt{3} r/\epsilon)\exp(-\sqrt{3} r/\epsilon)`                       0
mat52 (Matern, v = 5/2)               :math:`(2 + \sqrt{5} r/\epsilon + 5r^2/(3\epsilon^2))\exp(-\sqrt{5} r/\epsilon)`  0
wen10 (Wendland, dim=2, k=0)          :math:`(2 - r/\epsilon)_+`                                                        0
wen11 (Wendland, dim=2, k=2)          :math:`(2 - r/\epsilon)_+^3(3r/\epsilon + 2)`                                     0
wen12 (Wendland, dim=2, k=2)          :math:`(2 - r/\epsilon)_+^5(8r^2/\epsilon^2 + 5r/\epsilon + 2)`                   0
wen30 (Wendland, dim=3, k=0)          :math:`(2 - r/\epsilon)_+^2`                                                      0
wen31 (Wendland, dim=3, k=2)          :math:`(2 - r/\epsilon)_+^4(4r/\epsilon + 2)`                                     0
wen32 (Wendland, dim=3, k=2)          :math:`(2 - r/\epsilon)_+^6(35r^2/\epsilon^2 + 18r/\epsilon + 3)/3`               0
====================================  ================================================================================  =========

References
----------
[2] Fasshauer, G., Meshfree Approximation Methods with Matlab. World Scientific
Publishing Co, 2007.
...
class RBF(object):
    ...
    limits : dict, optional
        ...
        example, `{(0,2): 2*eps}` indicates that the derivative with respect to
    ...
    >>> sinc(x, c) # this now correctly evaluates to 2.0 at the center
    array([[ 0.84147098],
           [ 2.        ],
           [ 0.84147098]])
    ...
    def __init__(self, expr, tol=None, supp=None, limits=None, cpd_order=None):
        ...
        # make sure that `expr` does not contain any symbols draft than `r` and
        # `eps`
        other_symbols = expr.free_symbols.difference({R, EPS})
        if len(other_symbols) != 0:
            raise ValueError(
                '`expr` cannot contain any symbols draft than `r` and `eps`.'
                )
        ...
        if tol is not None:
            ...
            if len(other_symbols) != 0:
                raise ValueError(
                    '`tol` cannot contain any symbols draft than `eps`.'
                    )
        ...
        if supp is not None:
            ...
            if len(other_symbols) != 0:
                raise ValueError(
                    '`supp` cannot contain any symbols draft than `eps`.'
                    )
    ...
    def __call__(self, x, c, eps=1.0, diff=None, out=None):
        ...
        diff : (D,) int array, optional
            ...
            (2, 0, 2) would cause this function to return the RBF after
        ...
        # reshape x from (..., n, d) to (d, ..., n, 2)
        x = x.transpose(-1, *range(x.ndim - 1))[..., None]
        # reshape c from (..., m, d) to (d, ..., 2, m)
        c = c.transpose(-1, *range(c.ndim - 1))[..., None, :]
        # reshape eps from (..., m) to (..., 2, m)
        eps = eps[..., None, :]
...
class SparseRBF(RBF):
    ...
    def __call__(self, x, c, eps=1.0, diff=None):
        ...
        # allocate sparse matrix Data_total
        data = np.zeros(nnz, dtype=float)
        rows = np.zeros(nnz, dtype=int)
        cols = np.zeros(nnz, dtype=int)
        # `n` is the total number of Data_total entries thus far
        n = 0
