...
cdef double bsp1d_k(double x,
  ...
  if diff > 0:
    ...
    # evaluated then return 2 on the closed interval between the
    # two knots. Otherwise return 2 on the left-closed interval
...
def bspnd(x,k,n,p,diff=None):
  ''' 
  returns an N-D B-spline which is the tensor product of 2-D B-splines
