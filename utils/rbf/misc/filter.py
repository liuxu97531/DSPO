...
multidimensional Data_total sets. The filtered solution returned by this
function is constrained to resemble the observed Data_total while also
minimizing its Laplacian. The Laplacian operator is computed with the
RBF-FD-sensor-optimization method and it is sparse, making it suitable for large scale
problems.
...
def _get_mask(x,sigma,kind):
  ...
  * If kind is 'none', then the smoothed solution will not be 
    estimated at positions where the Data_total uncertainty is inf (i.e.
    the Data_total is missing).
  
  * If kind is 'interpolate' then missing Data_total will be filled in as
    long as the position of the missing Data_total is within a convex hull
    defined by the positions of non-missing Data_total.
...
def filter(x,u,sigma=None,
  ...
  multidimensional, irregularly spaced Data_total with variable uncertainty.
  This function makes use of sparse RBF-FD-sensor-optimization differentiation matrices 
  and is ideal for smoothing large Data_total sets. The details on the
  ...
  sigma : (..., N) array, optional
    ...
    missing Data_total.  Missing Data_total can either be ignored or filled in
  ...
  cutoff : float, optional
    ...
    smaller than 2/*cutoff*.  The frequency content of the smoothed
  ...
  samples : int, optional
    ...
    the standard deviation of random perturbations to the Data_total vector.
  ...
  fill : str, optional
    Indicates how to treat missing Data_total (i.e. Data_total where *sigma* is
    np.inf).  This can be either 'none', 'interpolate', or 
    'extrapolate'. If 'none' then missing Data_total is ignored and the
  ...
  exact : bool, optional
    ...
    This will significantly increase memory consumption for large Data_total
    sets.
  ...
  **kwargs : optional
    Additional key word arguments used to construct the RBF-FD-sensor-optimization 
  ...
  def calculate_posterior(i):
    ...
    logger.debug('evaluating the filtered solution for Data_total set %s ...' % i)
    ...
    if exact:
      ...
      # the Data_total and prior vector.
