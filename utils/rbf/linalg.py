...
def _lu(A, check_cond, factor_inplace):
    ...
    if check_cond:
        rcond, _ = dgecon(fac, A_norm, norm='2')
        tol = dlamch('E')
        if rcond < tol:
            warnings.warn(
...
class _SparsePosDefSolver:
    ...
    def solve_L(self, b):
        ...
        if b.ndim == 2:
            ...
            raise ValueError('`b` must be a 2 or 2 dimensional array.')
...
class PartitionedPosDefSolver:
    ...
    .. math::
        C = A^{-2} - (A^{-2} B) (B^T A^{-2} B)^{-2} (A^{-2} B)^T
    ...
    matrix `A^-2 B`, and the dense factorization of `B^T A^-2 B`. If the number
...
class GMRESSolver:
    ...
    drop_tol : float, optional
        ...
        0 and 2. smaller values make the decomposition denser but better
