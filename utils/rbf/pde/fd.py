'''
This module provides functions for generating RBF-FD-sensor-optimization weights
'''
...
# from ..basis import phs3, get_rbf
# from ..poly import monomial_count, monomial_powers, mvmonos
# from ..utils import assert_shape, KDTree
...
def weights(x, s, diffs, coeffs=None, phi=phs3, order=None, eps=1.0):
    ...
    computed using the RBF-FD-sensor-optimization method described in [2]
    ...
    order : int, optional
        ...
        order. For example, if `diffs` is `[[2, 0], [0, 2]]`, then this is set
        to 2.
    ...
    -------
    (..., M) float array
        RBF-FD-sensor-optimization weights for each target point
    ...
    References
    ----------
    [2] Fornberg, B. and N. Flyer. A Primer on Radial Basis Functions with
...
def weight_matrix(x, p, n, diffs,
    ...
    function which first creates stencils and then computes the RBF-FD-sensor-optimization weights
    for each stencil.
