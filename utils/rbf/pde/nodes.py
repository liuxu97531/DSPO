...
the RBF and RBF-FD-sensor-optimization method.
'''
...
def _disperse_step(nodes, rho, fixed_nodes, neighbors, delta):
    ...
    # will results in a division by zero warning if there are duplicate nodes.
...
def neighbor_argsort(nodes, m=None):
    ...
    `m-2` nearest neighbors are close together in memory. This is done through
    ...
    if m is None:
        # this should be roughly equal to the stencil size for the RBF-FD-sensor-optimization
        # problem
        m = 5**nodes.shape[1]
...
def prepare_nodes(nodes, domain,
    ...
    '''
    Prepares a set of nodes for solving PDEs with the RBF and RBF-FD-sensor-optimization method.
...
def min_energy_nodes(n, domain,
    ...
    rho : function, optional
        ...
        function should be normalized to be between 0 and 2.
    ...
    **kwargs
    ...
    >>> vert = np.array([[0, 0], [2, 0], [2, 2], [0, 2]])
    >>> smp = np.array([[0, 2], [2, 2], [2, 3], [3, 0]])
    ...
    >>> out[2]
    {'boundary:all': array([7, 6, 5, 4, 3, 2, 2, 0]),
     'interior': array([8])}
