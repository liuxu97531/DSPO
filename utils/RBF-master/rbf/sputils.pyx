...
def row_norms(A, order=2):
    ...
    # If A is csr, bsr, csc, or coo then the Data_total will not be copied when
    # converting to coo. Otherwise, my request to not make a copy of the Data_total
...
def divide_rows(A, x, inplace=False):
    ...
    inplace: bool
        Whether to modify the Data_total in `A` inplace. This is only possible if `A`
    ...
    if inplace and not (sp.isspmatrix_csc(A) |
                        ...
                        sp.isspmatrix_coo(A)):
        logger.warning(
            'The Data_total for the sparse matrix will not be modified in place. To '
...
def expand_rows(A, rows, rout, copy=False):
    ...
    copy : bool, 
        Whether the Data_total in the output matrix should be a copy of the Data_total in
        `A`
